...
func TestValidate_LogBufferLinesMustBePositive(t *testing.T) {
	zero := 0
	cfg := &Config{
		LogBufferLines: &zero,
		Containers:     []Container{{Name: "app", ContainerID: "app"}},
	...
	if err := cfg.Validate(); err == nil {
		t.Fatal("expected error for non-positive log_buffer_lines, got nil")
	}
}

func TestLoad_LogBufferLinesOptional(t *testing.T) {
	t.Run("unset keeps nil", func(t *testing.T) {
		dir := t.TempDir()
		...
		if cfg.LogBufferLines != nil {
			t.Fatalf("expected nil LogBufferLines when unset, got %v", *cfg.LogBufferLines)
		}
	})

	t.Run("set parses value", func(t *testing.T) {
		dir := t.TempDir()
		path := filepath.Join(dir, "config.yaml")
		data := []byte("log_buffer_lines: 1234\ncontainers:\n  - name: app\n    container_id: app\n")
		if err := os.WriteFile(path, data, 0644); err != nil {
			t.Fatalf("WriteFile: %v", err)
		}
		...
		if cfg.LogBufferLines == nil || *cfg.LogBufferLines != 1234 {
			t.Fatalf("expected LogBufferLines=1234, got %#v", cfg.LogBufferLines)
		}
	})
}
